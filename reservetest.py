...
EARLIEST_TIME = "6:00 PM"
LATEST_TIME = "9:30 PM"
RESERVATION_TIME_MIN = datetime.strptime(
    EARLIEST_TIME, RESERVATION_TIME_FORMAT)
...
EXTENSION_PATH = USER_DATA_DIR + '/' + PROFILE_DIR + \
...
class ReserveTFL():
    ...
    def reserve(self):
        ...
        print("Looking for availability on month: %s, days: %s, between times: %s and %s" % (
            RESERVATION_MONTH, RESERVATION_DAYS, EARLIEST_TIME, LATEST_TIME))
        ...
        while not RESERVATION_FOUND:
            ...
            self.driver.get("https://www.exploretock.com/blackburn-social-house-toronto/search?date=%s-%s-02&size=%s&time=%s" %
                            (RESERVATION_YEAR, month_num(RESERVATION_MONTH), RESERVATION_SIZE, "22%3A00"))
            WebDriverWait(self.driver, WEBDRIVER_TIMEOUT_DELAY_MS).until(
            ...
            WebDriverWait(self.driver, WEBDRIVER_TIMEOUT_DELAY_MS).until(expected_conditions.presence_of_element_located(
    ...
    def login_tock(self):
        ...
        WebDriverWait(self.driver, WEBDRIVER_TIMEOUT_DELAY_MS).until(
            expected_conditions.visibility_of_element_located((By.CSS_SELECTOR, ".MainHeader-accountName")))
    ...
    def search_time(self):
        for item in self.driver.find_elements(By.CSS_SELECTOR, "button.Consumer-resultsListItem.is-available"):
            ...
            available_time = datetime.strptime(
                span2.text, RESERVATION_TIME_FORMAT)
